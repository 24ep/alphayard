  ...
  async sendEmail(options: EmailOptions): Promise<boolean> {
    ...
    try {
      ...
      const enrichedData: Record<string, any> = {
        ...
        appName: options.data.appName || 'Boundary',
      };
      ...
      <html>
        ...
        <body>
          <div class="container">
            ...
            <div class="footer">
              <p>© 2024 Boundary. All rights reserved.</p>
            </div>
          </div>
        </body>
      </html>
  ...
  private generateContentFromData(data: Record<string, any>): string {
    ...
    if (data.inviterName && data.circleName) {
      content = `
        ...
        <p>Hello!</p>
        <p>${data.inviterName} has invited you to join their circle group "${data.circleName}" on Boundary.</p>
      ...
      content = `
        <h2>Welcome to Boundary, ${data.userName}!</h2>
      ...
      content = `
        ...
        <p>You requested a password reset for your Boundary account.</p>
  ...
  async sendTestEmail(to: string, type: 'mobile' | 'admin' = 'mobile'): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      await transporter.sendMail({
        from: smtpConfig.from || smtpConfig.user,
        to,
        subject: 'Test Email - Boundary SMTP Configuration',
        html: `
