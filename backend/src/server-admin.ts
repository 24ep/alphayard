...
console.log(`[Admin Server] Initialization trigger: ${new Date().toISOString()}`);
...
if (isProduction && cluster.isPrimary) {
  ...
  console.log(`🚀 Starting Bondarys Admin Backend in cluster mode with ${numCPUs} workers`);
...
function startServer() {
  ...
  // Redundant but preserving original overrides
  ...
  async function initializeServices() {
    try {
      ...
      const PORT = 3001;
      server.listen(PORT, '::', () => {
        console.log(`🚀 Bondarys Admin Backend Server running on port ${PORT}`);
      });
    } catch (error) {
