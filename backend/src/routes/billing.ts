...
// @access  Private
router.post('/create-subscription', authenticateToken as any, validateSubscription, async (req: any, res: Response) => {
  try {
    ...
    const { rows: subRows } = await pool.query(
      `INSERT INTO subscriptions (
        user_id, family_id, stripe_subscription_id, stripe_customer_id, 
...
// @access  Private
router.put('/subscription', authenticateToken as any, [
  body('planId').notEmpty(),
...
// @access  Private
router.delete('/subscription', authenticateToken as any, async (req: any, res: Response) => {
  try {
    const userId = req.user.id;
...
// @access  Private
router.post('/reactivate-subscription', authenticateToken as any, async (req: any, res: Response) => {
  try {
    const userId = req.user.id;
...
router.post('/apply-coupon', authenticateToken as any, [ body('coupon').notEmpty() ], async (req: any, res: Response) => {
  try {
    ...
    res.json({ message: 'Coupon applied', subscription: updatedRows[0] });
  } catch (error: any) {
