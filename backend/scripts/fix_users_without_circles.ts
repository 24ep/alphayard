...
async function fixUsersWithoutCircles() {
  ...
  try {
    ...
    const users = await prisma.$queryRaw<Array<{ id: string; email: string; first_name: string }>>(Prisma.sql`
    ...
    for (const user of users) {
      ...
      try {
        await prisma.$transaction(async (tx) => {
          ...
          const circles = await tx.$queryRaw<Array<{ id: string }>>(Prisma.sql`
            INSERT INTO circles (name, description, created_by, owner_id, invite_code, type, created_at, updated_at)
            VALUES (${'My Circle'}, ${'My personal circle'}, ${user.id}, ${user.id}, ${inviteCode}, ${'circle'}, NOW(), NOW())
            RETURNING id
          `);
          ...
          await tx.$executeRaw(Prisma.sql`
            ...
            VALUES (${circleId}, ${user.id}, ${'owner'}, NOW())
          `);
        });
