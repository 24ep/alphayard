...
async function main() {
  ...
  try {
    ...
    const res = await query(`SELECT raw_user_meta_data FROM users WHERE id = $1`, [userId]);
    if (res.rows.length === 0) {
      console.log('User not found');
      process.exit(1);
    }
    
    const currentMeta = res.rows[0].raw_user_meta_data || {};
    ...
    // 3. Verify
    const verifyRes = await query(`SELECT raw_user_meta_data FROM users WHERE id = $1`, [userId]);
    console.log('New metadata:', verifyRes.rows[0].raw_user_meta_data);
