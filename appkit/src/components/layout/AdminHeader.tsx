...
export function AdminHeader({ 
    ...
    useEffect(() => {
        const loadBranding = async () => {
            try {
                const res = await fetch('/api/v1/admin/config/branding')
                if (res.ok) {
                    const data = await res.json()
                    const branding = data?.branding || data
                    if (branding?.logoUrl) setAppLogoUrl(branding.logoUrl)
                    if (branding?.adminAppName || branding?.name) setAppBrandName(branding.adminAppName || branding.name || 'AppKit')
                }
            } catch {}
        }
        loadBranding()
