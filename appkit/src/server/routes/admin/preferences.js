...
router.get('/:key', authenticateAdmin, async (req, res) => {
    try {
        ...
        const rows = await prisma.$queryRawUnsafe(`SELECT preference_value FROM admin_user_preferences WHERE user_id = $1 AND preference_key = $2`, userId, key);
        if (rows.length === 0) {
...
router.put('/:key', authenticateAdmin, async (req, res) => {
    try {
        ...
        await prisma.$executeRawUnsafe(`INSERT INTO admin_user_preferences (user_id, preference_key, preference_value)
             VALUES ($1, $2, $3)
             ON CONFLICT (user_id, preference_key) 
             DO UPDATE SET preference_value = $3, updated_at = NOW()`, userId, key, JSON.stringify(value));
...
router.delete('/:key', authenticateAdmin, async (req, res) => {
    try {
        ...
        await prisma.$executeRawUnsafe(`DELETE FROM admin_user_preferences WHERE user_id = $1 AND preference_key = $2`, userId, key);
...
router.get('/', authenticateAdmin, async (req, res) => {
    try {
        ...
        const rows = await prisma.$queryRawUnsafe(`SELECT preference_key, preference_value FROM admin_user_preferences WHERE user_id = $1`, userId);
        const preferences = {};
        rows.forEach(row => {
