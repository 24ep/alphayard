...
class DatabaseExplorerService {
    ...
    async getTables(): Promise<TableInfo[]> {
        const result = await prisma.$queryRawUnsafe<any[]>(`
            ...
            WHERE t.table_schema IN ('admin', 'public')
    ...
    async getDatabaseStats(): Promise<{
        ...
        const [sizeResult, tableCountResult, connectionsResult, uptimeResult] = await Promise.all([
            ...
            prisma.$queryRawUnsafe<any[]>(`SELECT COUNT(*) as count FROM information_schema.tables WHERE table_schema IN ('admin', 'public') AND table_type = 'BASE TABLE'`),
