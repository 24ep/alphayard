...
import { View, Text, StyleSheet, Dimensions, Image, TouchableOpacity, Alert, Modal, ScrollView } from 'react-native';
...
export const FreeMapView: React.FC<FreeMapViewProps> = ({ 
  ...
  const getLocationLabel = (lat: number, lng: number, userName: string): string => {
          ...
          <View style={styles.drawerFamilyCardsContainer}>
            ...
            >
              {locations.map((member, index) => {
                const currentUser = locations[0]; // First location is current user
