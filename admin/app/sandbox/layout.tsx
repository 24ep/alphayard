...
import { SandboxSidebarVertical } from '@components/layout/SandboxSidebarVertical'
...
const navigationHubs: NavHub[] = [
    ...
    },
    {
        ...
        href: '/sandbox',
        items: [
            { id: 'sandbox', label: 'Sandbox Home', href: '/sandbox', icon: 'server' },
            { id: 'api-playground', label: 'API Playground', href: '/sandbox/api-playground', icon: 'code' },
            { id: 'ab-testing', label: 'A/B Testing', href: '/sandbox/ab-testing', icon: 'activity' },
            { id: 'analytics', label: 'Analytics', href: '/sandbox/analytics', icon: 'chart-bar' },
            { id: 'config-sync', label: 'Config Sync', href: '/sandbox/config-sync', icon: 'settings' },
            { id: 'device-testing', label: 'Device Testing', href: '/sandbox/device-testing', icon: 'smartphone' },
            { id: 'performance', label: 'Performance', href: '/sandbox/performance', icon: 'zap' },
            { id: 'security', label: 'Security Testing', href: '/sandbox/security', icon: 'shield' },
            { id: 'accessibility', label: 'Accessibility', href: '/sandbox/accessibility', icon: 'users' },
            { id: 'tools', label: 'Tools', href: '/sandbox/tools', icon: 'layout' },
            { id: 'integration-guide', label: 'Integration Guide', href: '/sandbox/integration-guide', icon: 'document-text' }
        ]
    },
    {
        id: 'content',
...
const Icon = ({ name, className = 'w-5 h-5' }: { name: string; className?: string }) => {
    const icons: Record<string, JSX.Element> = {
        ...
        'smartphone': <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="5" y="2" width="14" height="20" rx="2" ry="2" strokeWidth={2} /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 18h.01" /></svg>,
        'zap': <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" /></svg>
    }
    return icons[name] || <span className={className}>•</span>
}

// Inner layout component that uses permissions
function SandboxLayoutInner({ children }: SandboxLayoutProps) {
    ...
    const [sandboxSidebarOpen, setSandboxSidebarOpen] = useState(true)
    ...
    const activeHub = useMemo(() => {
        // First check if we're on sandbox pages
        if (pathname.startsWith('/sandbox')) {
            const sandboxHub = filteredNavigationHubs.find(h => h.id === 'sandbox')
            if (sandboxHub) return sandboxHub
        }
        
        // Otherwise check other hubs
        return filteredNavigationHubs.find(hub => pathname.startsWith(hub.href)) || filteredNavigationHubs[0]
    }, [pathname, filteredNavigationHubs])

    const getModuleTitle = () => {
        if (!activeHub) return 'Sandbox'
    ...
    return (
        <Fragment>
            ...
            <header className="h-16 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border-b border-gray-200/50 dark:border-zinc-800/50 flex items-center justify-between px-4 lg:px-8 relative z-50 shrink-0 shadow-sm shadow-gray-200/20 dark:shadow-zinc-900/20">
                <div className="flex items-center space-x-6">
                    ...
                    <div className="flex items-center group">
                        <div className="w-9 h-9 bg-gradient-to-br from-blue-600 via-blue-600 to-blue-700 rounded-xl flex-shrink-0 flex items-center justify-center font-bold text-white text-sm shadow-lg shadow-blue-600/25 group-hover:shadow-blue-600/40 transition-all duration-300">
                            A
                        </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                {/* 1. Primary Sidebar */}
                <aside 
                    className="hidden lg:flex flex-col bg-white dark:bg-black border-r border-gray-200 dark:border-gray-800 z-40 flex-shrink-0 w-12"
                >
                            ...
                            return (
                                <Tooltip key={hub.id} content={hub.label} position="right">
                                    <button
                                        ...
                                        className={`group transition-all duration-200 w-8 h-8 flex items-center justify-center ${
                                            isHubActive
                                                ? 'bg-blue-600 text-white'
                                                : 'text-gray-500 hover:text-gray-300 hover:bg-gray-800'
                                        }`}
                                    >
                                        <Icon name={hub.icon} className={`w-4 h-4 flex-shrink-0 transition-all duration-200 ${isHubActive ? 'text-white' : ''}`} />
                                        {isHubActive && (
                    ...
                    <div className="py-2 flex flex-col items-center space-y-1">
                        <Tooltip content="Sandbox" position="right">
                            <button 
                                ...
                                className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-purple-400 hover:bg-gray-800 transition-all duration-200"
                        ...
                        <Tooltip content="Developer Hub" position="right">
                            <button 
                                ...
                                className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-gray-300 hover:bg-gray-800 transition-all duration-200"
                ...
                {/* 2. Secondary Sidebar */}
                <aside className="hidden lg:flex flex-col w-48 bg-white dark:bg-black border-r border-gray-200 dark:border-gray-800 z-30 flex-shrink-0">
                    ...
                    <nav className="flex-1 overflow-y-auto py-2">
                        {(() => {
                            // Group items by group name for App Content hub
                            if (activeHub.id === 'content') {
                                        ...
                                        {items.map((item: NavItem) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center px-3 py-1.5 text-xs font-mono transition-all duration-200 ${
                                                        isActive
                                                            ? 'bg-blue-600 text-white'
                                                ...
                                                >
                                                    <Icon 
                                                        name={item.icon}
                                                        className={`w-3 h-3 mr-2 transition-colors ${isActive ? 'text-white' : 'text-gray-500 dark:text-gray-400'}`} 
                                                    />
                                                    {item.label}
                                                </button>
                                            )
...
export default function SandboxLayout({ children }: SandboxLayoutProps) {
    return (
        <PermissionProvider>
            <SandboxLayoutInner>{children}</SandboxLayoutInner>
        </PermissionProvider>
    )
}
